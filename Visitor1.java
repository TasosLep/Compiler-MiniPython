	...
	public void inAFunction(AFunction node)
    {
		...
		//	System.out.println(other.getId().toString());
			if (other_args.size() == args.size()){
				...
				if(!(arg1.getId()).equals(arg2.getId())){ //elegxei an to onoma tou identifier tou enos einai idio me tou allou
				...
				if (list1.size() == list2.size()){ // an einai idio to mege8os ton liston
					if (list1.size() == 1){
				...
				if(list1.size() == list2.size()){
					...
					for(int i = 0; i < list1.size(); i++){
						...
						if(!(commaid1.getId()).equals(commaid2.getId())){
							symtable.put(fName, node);
							return;
						}
						else{
